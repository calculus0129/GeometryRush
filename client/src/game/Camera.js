export class Camera {
  ...
  follow(player) {
    ...
    this.targetX = player.x - 2; // Keep player slightly left of center
    this.targetY = player.y - 1; // Keep player slightly below center
  ...
  worldToScreen(worldX, worldY, canvasWidth, canvasHeight) {
    ...
    const screenY = canvasHeight - ((worldY - this.y) / this.height) * canvasHeight;
